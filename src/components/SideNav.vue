  ...
  <div class="flex flex-col h-full bg-black/75 backdrop-blur-md border-r border-white/10 shadow-2xl">
    ...
    <div class="flex items-center gap-3 px-6 py-6 border-b border-white/10">
      ...
      <div>
        ...
        <p class="text-cinema-text-muted text-xs">Der Filmclub</p>
      </div>
    </div>
    ...
    <div class="px-4 py-6 border-t border-white/10 space-y-3">
      <!-- Logged in state -->
      <div v-if="isAuthenticated && user" class="flex items-center gap-3 px-4 py-3">
        ...
        <div class="flex-1 min-w-0">
          <p class="text-cinema-text text-sm font-medium truncate">{{ user }}</p>
          <span v-if="isAdmin" class="text-xs text-cinema-red font-medium">Admin</span>
        </div>
      </div>

      <!-- Not logged in state (Phase 1 stub) -->
      <router-link
        v-else
        to="/login"
      ...
      <!-- Admin: Add Film Button (only visible when isAdmin: true) -->
      <button
        v-if="isAdmin"
